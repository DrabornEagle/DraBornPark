...
export function ScreenHeader({title,eyebrow='DraBornPark',right,accent=palette.cyan,back=true,subtitle}:{title:string;eyebrow?:string;right?:React.ReactNode;accent?:string;back?:boolean;subtitle?:string}){
  return <View style={[s.headerWrap,{borderColor:`${accent}52`}]}> 
    <View style={s.header}>
      ...
      <View style={{flex:1,minWidth:0}}>{eyebrow?<Text numberOfLines={1} style={[s.eyebrow,{color:accent}]}>{eyebrow}</Text>:null}<Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.62} style={s.headerTitle}>{title}</Text>{subtitle?<Text style={s.headerSubtitle}>{subtitle}</Text>:null}</View>
