...
if(expected!=='0.5.3')fail.push(`Release audit expected 0.5.3 but found ${expected}.`);
if(!Number.isInteger(app.expo?.android?.versionCode)||app.expo.android.versionCode<19)fail.push('Android versionCode must be >=19 for v0.5.3.');
...
for(const marker of ["import('expo-notifications')",'getExpoPushTokenAsync','drabornpark_push_tokens','drabornpark-alerts-v3','drabornpark_messages','presentLocalVisitorMessage','scheduleNotificationAsync','initializeNotificationPresentation','CATEGORY_TITLES','pollMessages'])if(!push.includes(marker))fail.push(`v0.5.3 push marker missing: ${marker}`);
