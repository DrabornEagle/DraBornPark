...
function ReportCard({report,messages,session,sending,draft,setDraft,onSeen,onReply}:{report:any;messages:ContactMessage[];session?:ContactSession;sending:boolean;draft:string;setDraft:(value:string)=>void;onSeen:()=>void;onReply:(text:string)=>void}){
  ...
  return <View style={[s.card,{borderColor:!report.seen_at?`${color}65`:palette.line}]}>
    ...
    {open?<><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.replyRail}>{QUICK_REPLIES.map(text=><Pressable disabled={sending} key={text} onPress={()=>onReply(text)} style={s.reply}>{sending?<ActivityIndicator size="small" color={palette.green}/>:<Text style={s.replyText}>{text}</Text>}</Pressable>)}</ScrollView><View style={s.compose}><TextInput value={draft} onChangeText={setDraft} editable={!sending} maxLength={700} placeholder="Kısa bir mesaj yaz…" placeholderTextColor={palette.muted2} style={s.input}/><Pressable disabled={sending||!draft.trim()} onPress={()=>onReply(draft)} style={[s.sendButton,(!draft.trim()||sending)&&s.sendButtonDisabled]}>{sending?<ActivityIndicator size="small" color={palette.bg}/>:<MaterialCommunityIcons name="send" size={20} color={palette.bg}/>}</Pressable></View></>:null}
  </View>;
}
