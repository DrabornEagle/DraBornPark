...
export default function HubScreen(){
  ...
  return <SafeAreaView style={s.safe}><AuroraBackground accent={palette.purple} secondary={palette.cyan}/><Animated.View style={{flex:1,opacity:enter,transform:[{translateY:enter.interpolate({inputRange:[0,1],outputRange:[12,0]})}]}}><ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <ScreenHeader back={false} title="DraBorn Merkezi" eyebrow="NOVA RENK SİSTEMİ" accent={palette.purple} subtitle="Araç, park, güvenlik ve hesabın tek anlaşılır merkezde" right={<Pill label="v0.4.4" color={palette.purple}/>}/>
    <View style={s.hero}>
      <View style={s.heroBadge}><SafeIcon name="view-dashboard-outline" size={36} color={palette.purple}/><View style={s.heroDot}/></View>
      <View style={{flex:1}}><Text style={s.heroKicker}>{demo.active?'ETKİLEŞİMLİ DEMO':'KİŞİSEL ARAÇ AĞIN'}</Text><Text style={s.heroTitle}>Ne yapmak istiyorsun?</Text><Text style={s.heroText}>Her işlev kendi rengi ve ikonu ile ayrılıyor. Ara veya doğrudan dokun.</Text><View style={s.heroSpectrum}><View style={[s.spectrum,{backgroundColor:palette.cyan}]}/><View style={[s.spectrum,{backgroundColor:palette.purple}]}/><View style={[s.spectrum,{backgroundColor:palette.pink}]}/><View style={[s.spectrum,{backgroundColor:palette.orange}]}/><View style={[s.spectrum,{backgroundColor:palette.green}]}/></View></View>
    </View>
    ...
    {!query?<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.categoryRail}>{groups.map((g,index)=><View key={g.title} style={[s.category,{borderColor:`${g.color}82`,backgroundColor:`${g.color}20`}]}><View style={[s.categoryIcon,{backgroundColor:`${g.color}32`,borderColor:`${g.color}88`}]}><SafeIcon name={g.icon} size={25} color={g.color}/></View><Text style={s.categoryTitle}>{g.title}</Text><View style={s.categoryBottom}><Text style={s.categoryCount}>{g.items.length} özellik</Text><Text style={[s.categoryNo,{color:g.color}]}>0{index+1}</Text></View><View style={[s.categoryLine,{backgroundColor:g.color}]}/></View>)}</ScrollView>:null}
...
function MenuTile({item,index,onPress}:{item:MenuItem;index:number;onPress:()=>void}){
  ...
  useEffect(()=>{Animated.timing(appear,{toValue:1,duration:380,delay:Math.min(index,16)*28,easing:Easing.out(Easing.cubic),useNativeDriver:true}).start()},[appear,index]);
  const press=(value:number)=>Animated.spring(scale,{toValue:value,useNativeDriver:true,speed:36,bounciness:4}).start();
  return <Animated.View style={[s.tileWrap,{opacity:appear,transform:[{translateY:appear.interpolate({inputRange:[0,1],outputRange:[12,0]})},{scale}]}]}><Pressable onPress={onPress} onPressIn={()=>press(.965)} onPressOut={()=>press(1)} style={[s.tile,{borderColor:`${item.color}88`,backgroundColor:`${item.color}21`}]}> 
    <View style={s.tileTop}><View style={[s.tileIcon,{backgroundColor:`${item.color}35`,borderColor:`${item.color}92`}]}><SafeIcon name={item.icon} size={32} color={item.color}/><View style={[s.tileIconDot,{backgroundColor:item.color}]}/><View style={[s.tileIconBar,{backgroundColor:item.color}]}/></View>{item.badge?<View style={[s.tileBadge,{backgroundColor:`${item.color}28`,borderColor:`${item.color}78`}]}><Text style={[s.tileBadgeText,{color:item.color}]}>{item.badge}</Text></View>:<View style={[s.tileArrow,{backgroundColor:`${item.color}22`,borderColor:`${item.color}68`}]}><SafeIcon name="arrow-top-right" size={18} color={item.color}/></View>}</View>
...
const s=StyleSheet.create({
  ...
  hero:{minHeight:180,borderRadius:radius.xl,borderWidth:1,borderColor:`${palette.purple}82`,backgroundColor:`${palette.purple}1C`,padding:20,flexDirection:'row',alignItems:'center',gap:16,overflow:'hidden'},heroBadge:{width:78,height:78,borderRadius:27,borderWidth:1,borderColor:`${palette.purple}98`,backgroundColor:`${palette.purple}32`,alignItems:'center',justifyContent:'center'},heroDot:{position:'absolute',width:10,height:10,borderRadius:5,right:8,top:8,backgroundColor:palette.aqua},heroKicker:{color:palette.cyan,fontSize:type.micro,fontWeight:'900',letterSpacing:1.25},heroTitle:{color:palette.text,fontSize:26,fontWeight:'900',letterSpacing:-.72,marginTop:5},heroText:{color:palette.muted,fontSize:type.caption,lineHeight:19,marginTop:7},heroSpectrum:{height:4,marginTop:13,flexDirection:'row',gap:3},spectrum:{flex:1,borderRadius:99},
  search:{height:64,borderRadius:23,borderWidth:1,borderColor:`${palette.cyan}68`,backgroundColor:palette.glassStrong,marginTop:14,paddingHorizontal:12,flexDirection:'row',alignItems:'center',gap:10},searchIcon:{width:40,height:40,borderRadius:14,borderWidth:1,borderColor:`${palette.cyan}72`,backgroundColor:`${palette.cyan}22`,alignItems:'center',justifyContent:'center'},searchInput:{flex:1,color:palette.text,fontSize:type.body,paddingVertical:0},clear:{width:36,height:36,borderRadius:12,borderWidth:1,borderColor:palette.line,alignItems:'center',justifyContent:'center'},
  ...
  grid:{flexDirection:'row',flexWrap:'wrap',gap:10},tileWrap:{width:'48.4%'},tile:{minHeight:194,borderRadius:27,borderWidth:1,padding:14,overflow:'hidden'},tileTop:{flexDirection:'row',alignItems:'center',justifyContent:'space-between'},tileIcon:{width:58,height:58,borderRadius:19,borderWidth:1,alignItems:'center',justifyContent:'center',overflow:'hidden'},tileIconDot:{position:'absolute',width:7,height:7,borderRadius:4,right:7,top:7},tileIconBar:{position:'absolute',left:9,right:9,bottom:0,height:3,borderRadius:2},tileBadge:{borderWidth:1,borderRadius:999,paddingHorizontal:8,paddingVertical:6},tileBadgeText:{fontSize:11,fontWeight:'900',letterSpacing:.4},tileArrow:{width:34,height:34,borderRadius:12,borderWidth:1,alignItems:'center',justifyContent:'center'},tileTitle:{color:palette.text,fontSize:16.5,fontWeight:'900',lineHeight:21,marginTop:15},tileSub:{color:palette.muted,fontSize:12.5,lineHeight:18,marginTop:5,minHeight:36},tileFooter:{marginTop:'auto',paddingTop:12,flexDirection:'row',alignItems:'center',gap:6},tileOpen:{fontSize:11,fontWeight:'900',letterSpacing:.8},tileMiniLine:{flex:1,height:2,borderRadius:99,opacity:.7},tileBottom:{position:'absolute',height:5,left:14,right:14,bottom:0,borderRadius:3},
  ...
  reset:{minHeight:100,borderRadius:radius.lg,borderWidth:1,borderColor:`${palette.yellow}70`,backgroundColor:`${palette.yellow}18`,padding:15,flexDirection:'row',alignItems:'center',gap:13,marginTop:28},resetIcon:{width:48,height:48,borderRadius:16,borderWidth:1,borderColor:`${palette.yellow}78`,backgroundColor:`${palette.yellow}28`,alignItems:'center',justifyContent:'center'},resetTitle:{color:palette.text,fontSize:type.cardTitle,fontWeight:'900'},resetText:{color:palette.muted,fontSize:type.caption,lineHeight:19,marginTop:4},
});
