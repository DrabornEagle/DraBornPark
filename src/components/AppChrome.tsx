...
export function NotificationBell({countOverride}:{countOverride?:number}){
  ...
  return <Animated.View style={{opacity:active?1:.28,transform:[{scale:pulse.interpolate({inputRange:[0,1],outputRange:[1,1.1]})}]}}>
    <Pressable accessibilityLabel="Bildirimler" onPress={()=>router.push('/notifications')} style={({pressed})=>[s.bell,{borderColor:active?`${palette.pink}88`:'#FFFFFF20',backgroundColor:active?`${palette.pink}22`:'#FFFFFF08'},pressed&&s.pressed]}>
      <SafeIcon name={active?'bell-ring':'bell-outline'} size={23} color={active?palette.pink:palette.muted2}/>
      {active?<View style={s.bellBadge}><Text style={s.bellBadgeText}>{count>9?'9+':count}</Text></View>:null}
    </Pressable>
  </Animated.View>;
}
...
export function ScreenHeader({title,eyebrow='DraBornPark',right,accent=palette.cyan,back=true,subtitle}:{title:string;eyebrow?:string;right?:React.ReactNode;accent?:string;back?:boolean;subtitle?:string}){
  return <View style={[s.headerWrap,{borderColor:`${accent}52`}]}> 
    <View style={s.header}>
      ...
      <View style={{flex:1,minWidth:0}}>{eyebrow?<Text style={[s.eyebrow,{color:accent}]}>{eyebrow}</Text>:null}<Text style={s.headerTitle}>{title}</Text>{subtitle?<Text style={s.headerSubtitle}>{subtitle}</Text>:null}</View>
      <View style={s.headerRight}>{right??null}<NotificationBell/></View>
    </View>
...
export function BottomDock({active='home',onHub,transparent=false,floating=false}:{active?:'home'|'park'|'inbox'|'hub';onHub?:()=>void;transparent?:boolean;floating?:boolean}){
  const fade=useRef(new Animated.Value(transparent?0:1)).current;
  useEffect(()=>{Animated.timing(fade,{toValue:transparent?0:1,duration:240,easing:Easing.out(Easing.cubic),useNativeDriver:true}).start()},[fade,transparent]);
  const items=[
  ...
  const opacity=fade.interpolate({inputRange:[0,1],outputRange:[.42,1]});
  return <Animated.View pointerEvents="box-none" style={[s.dockWrap,floating&&s.dockFloating,{opacity}]}><View style={[s.dock,{backgroundColor:transparent?'#05081622':palette.glassStrong,borderColor:transparent?'#FFFFFF10':palette.line}]}><View pointerEvents="none" style={[s.dockTopSpectrum,{opacity:transparent?.25:1}]}><View style={[s.dockBar,{backgroundColor:palette.cyan}]}/><View style={[s.dockBar,{backgroundColor:palette.purple}]}/><View style={[s.dockBar,{backgroundColor:palette.pink}]}/><View style={[s.dockBar,{backgroundColor:palette.orange}]}/><View style={[s.dockBar,{backgroundColor:palette.green}]}/></View>{items.map(item=>{const selected=active===item.key;return <Pressable key={item.key} onPress={item.action} style={({pressed})=>[s.dockItem,selected&&!transparent&&{backgroundColor:`${item.color}20`,borderColor:`${item.color}62`},pressed&&{opacity:.72,transform:[{scale:.97}]}]}><View style={[s.dockIcon,selected&&!transparent&&{borderColor:`${item.color}88`,backgroundColor:`${item.color}30`}]}><SafeIcon name={item.icon} size={24} color={selected?item.color:palette.muted2}/>{selected&&!transparent?<View style={[s.activeDot,{backgroundColor:item.color}]}/>:null}</View><Text style={[s.dockLabel,selected&&{color:palette.text}]}>{item.label}</Text></Pressable>})}</View></Animated.View>;
}
