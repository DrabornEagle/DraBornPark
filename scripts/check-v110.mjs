...
if(!Object.values(dkd_pkg.scripts??{}).every(dkd_script=>String(dkd_script).includes('apply-v110-source-transform.mjs')&&String(dkd_script).includes('apply-v110-billing-hotfix.mjs')))dkd_fail.push('All project scripts must apply v1.0.10 source + billing hotfix transforms');
for(const dkd_file of ['google-services.json','assets/branding/icon.png','assets/branding/adaptive-icon.png','assets/branding/splash-icon.png','scripts/apply-v110-source-transform.mjs','scripts/apply-v110-billing-hotfix.mjs','src/components/DraBornParkPlusPanel.tsx','src/components/MandatoryUpdateGate.tsx','supabase/functions/dkd-drabornpark-app-version/index.ts'])if(!dkd_exists(dkd_file))dkd_fail.push('Required file missing: '+dkd_file);
...
for(const dkd_marker of ['dkdFinishOwnedPurchase','await finishTransaction({purchase:dkd_purchase,isConsumable:false})','getStorePurchases()','purchaseState','onEntitlementChanged','aktif Google Play aboneliğini kullanacak'])if(!dkd_plus.includes(dkd_marker))dkd_fail.push('Google Play acknowledgement/recovery marker missing: '+dkd_marker);
const dkd_finish_index=dkd_plus.indexOf('await finishTransaction({purchase:dkd_purchase,isConsumable:false})');
const dkd_verify_index=dkd_plus.indexOf("supabase.functions.invoke('dkd-drabornpark-google-play'");
if(dkd_finish_index<0||dkd_verify_index<0||dkd_finish_index>dkd_verify_index)dkd_fail.push('Google Play purchase must be acknowledged before remote verification can block completion');

const dkd_drabornpark=dkd_read('src/lib/drabornpark.ts');
for(const dkd_marker of ['dkdLoadLocalGooglePlaySubscription','getAvailablePurchases','finishTransaction','client_store_entitlement:true','dkdResolvedSubscription'])if(!dkd_drabornpark.includes(dkd_marker))dkd_fail.push('Local Play entitlement fallback marker missing: '+dkd_marker);
const dkd_feature=dkd_read('app/feature/[slug].tsx');
if(!dkd_feature.includes('onEntitlementChanged'))dkd_fail.push('Plus page must refresh entitlement after purchase acknowledgement');
...
console.log('DraBornPark integrity OK • v1.0.10 • Android vc10 • mandatory update • Premium popup/countdown • Google Play purchase acknowledgement before verification • startup purchase recovery • local active-subscription fallback • signed release gate ready.');
