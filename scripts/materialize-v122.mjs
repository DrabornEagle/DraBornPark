...
if(fs.existsSync('package-lock.json')){const dkd_lock=JSON.parse(dkd_read('package-lock.json'));dkd_lock.version='1.0.22';if(dkd_lock.packages?.[''])dkd_lock.packages[''].version='1.0.22';fs.writeFileSync('package-lock.json',JSON.stringify(dkd_lock,null,2)+'\n');}
fs.writeFileSync('.github/VERSION','1.0.22\n');
console.log('DraBornPark v1.0.22 materialization complete.');
