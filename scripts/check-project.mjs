...
for(const file of uiFiles){
  ...
  const regex=/fontSize\s*:\s*(\d+(?:\.\d+)?)/g;let match;
  while((match=regex.exec(src))){const size=Number(match[1]);if(size<11)fail.push(`Tiny text (${size}px) in ${path.relative(root,file)}. Use the shared readable type scale or >=11px.`)}
  if(!file.endsWith(path.join('src','components','Primitives.tsx'))){
    for(const legacy of ['car-plus','car-alert','shield-search-outline','map-marker-heart-outline']){
      if(src.includes(`'${legacy}'`)||src.includes(`"${legacy}"`))fail.push(`Legacy/invalid Material icon '${legacy}' in ${path.relative(root,file)}. Use SafeIcon alias target instead.`);
    }
  }
}
...
console.log(`DraBornPark integrity OK • v${expected} • ${routeFiles.length} routes • readable typography • release privacy/account checks verified.`);
