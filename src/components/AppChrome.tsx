...
export function BottomDock({active='home',onHub}:{active?:'home'|'park'|'inbox'|'hub';onHub?:()=>void}){
  ...
  return <View style={[s.dockWrap,{paddingBottom:Math.max(insets.bottom,14)}]}><View style={s.dock}><View pointerEvents="none" style={s.dockTopSpectrum}><View style={[s.dockBar,{backgroundColor:palette.cyan}]}/><View style={[s.dockBar,{backgroundColor:palette.purple}]}/><View style={[s.dockBar,{backgroundColor:palette.pink}]}/><View style={[s.dockBar,{backgroundColor:palette.orange}]}/><View style={[s.dockBar,{backgroundColor:palette.green}]}/></View>{items.map(item=>{const selected=active===item.key;return <Pressable key={item.key} onPress={item.action} style={({pressed})=>[s.dockItem,selected&&{backgroundColor:`${item.color}20`,borderColor:`${item.color}62`},pressed&&{opacity:.72,transform:[{scale:.97}]}]}><View style={[s.dockIcon,selected&&{borderColor:`${item.color}88`,backgroundColor:`${item.color}30`}]}><SafeIcon name={item.icon} size={24} color={selected?item.color:palette.muted2}/>{selected?<View style={[s.activeDot,{backgroundColor:item.color}]}/>:null}</View><Text style={[s.dockLabel,selected&&{color:palette.text}]}>{item.label}</Text></Pressable>})}</View></View>;
}
...
const s=StyleSheet.create({
  ...
  dockWrap:{paddingTop:18},dock:{minHeight:92,borderRadius:31,borderWidth:1,borderColor:palette.line,backgroundColor:palette.glassStrong,paddingHorizontal:7,paddingVertical:9,flexDirection:'row',alignItems:'center',overflow:'hidden'},dockTopSpectrum:{position:'absolute',left:18,right:18,top:0,height:3,flexDirection:'row',gap:3},dockBar:{flex:1,borderRadius:99},dockItem:{flex:1,minHeight:70,borderRadius:22,borderWidth:1,borderColor:'transparent',alignItems:'center',justifyContent:'center',gap:5},dockIcon:{width:46,height:42,borderRadius:16,borderWidth:1,borderColor:'transparent',alignItems:'center',justifyContent:'center'},activeDot:{position:'absolute',width:5,height:5,borderRadius:3,bottom:4},dockLabel:{fontSize:type.micro,color:palette.muted2,fontWeight:'900'},
