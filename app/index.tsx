...
function Loading(){
  ...
  return <SafeAreaView style={s.safe}><AuroraBackground accent={palette.cyan} secondary={palette.purple}/><View style={s.loading}>
    <View style={s.loadingStage}>
      ...
      <Animated.View style={[s.loadingCore,{transform:[{scale:pulse.interpolate({inputRange:[0,1],outputRange:[.97,1.045]})}]}]}>
        <Animated.View pointerEvents="none" style={[s.loadingCoreSweep,{transform:[{translateX:sweep.interpolate({inputRange:[0,1],outputRange:[-85,120]})},{rotate:'14deg'}]}]}/>
        <View style={s.loadingCoreIcon}><SafeIcon name="car-connected" size={46} color={palette.cyan}/><Animated.View style={[s.loadingCoreDot,{opacity:beat.interpolate({inputRange:[0,1],outputRange:[.45,1]}),transform:[{scale:beat.interpolate({inputRange:[0,1],outputRange:[1,1.35]})}]}]}/></View>
        <View style={s.loadingSignal}><View style={[s.loadingSignalBar,{height:8,backgroundColor:palette.blue}]}/><View style={[s.loadingSignalBar,{height:13,backgroundColor:palette.purple}]}/><View style={[s.loadingSignalBar,{height:18,backgroundColor:palette.cyan}]}/></View>
      </Animated.View>
    </View>
