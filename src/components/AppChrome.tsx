...
export function AuroraBackground({ accent = palette.cyan, secondary = palette.purple }: { accent?: string; secondary?: string }) {
  ...
  return <View pointerEvents="none" style={StyleSheet.absoluteFill}>
    <View style={s.base}/>
    <Animated.View style={[s.orbA,{backgroundColor:`${accent}38`,opacity:pulse.interpolate({inputRange:[0,1],outputRange:[.46,.86]}),transform:[{translateX:drift.interpolate({inputRange:[0,1],outputRange:[-44,34]})},{translateY:drift.interpolate({inputRange:[0,1],outputRange:[-30,38]})},{scale:pulse.interpolate({inputRange:[0,1],outputRange:[.96,1.1]})}]}]}/>
    <Animated.View style={[s.orbB,{backgroundColor:`${secondary}34`,opacity:pulse.interpolate({inputRange:[0,1],outputRange:[.3,.66]}),transform:[{translateX:drift.interpolate({inputRange:[0,1],outputRange:[42,-32]})},{translateY:drift.interpolate({inputRange:[0,1],outputRange:[28,-28]})}]}]}/>
    <Animated.View style={[s.orbC,{opacity:pulse.interpolate({inputRange:[0,1],outputRange:[.38,.72]})}]}/>
    <Animated.View style={[s.orbD,{opacity:pulse.interpolate({inputRange:[0,1],outputRange:[.28,.58]})}]}/>
    <Animated.View style={[s.orbE,{transform:[{translateY:drift.interpolate({inputRange:[0,1],outputRange:[26,-34]})}]}]}/>
    ...
    <Animated.View style={[s.spectrumRail,{transform:[{translateX:drift.interpolate({inputRange:[0,1],outputRange:[-120,46]})}]}]}><View style={[s.railDot,{backgroundColor:palette.cyan}]}/><View style={[s.railDot,{backgroundColor:palette.purple}]}/><View style={[s.railDot,{backgroundColor:palette.pink}]}/><View style={[s.railDot,{backgroundColor:palette.orange}]}/><View style={[s.railDot,{backgroundColor:palette.green}]}/></Animated.View>
  </View>;
}
...
export function ScreenHeader({title,eyebrow='DraBornPark',right,accent=palette.cyan,back=true,subtitle}:{title:string;eyebrow?:string;right?:React.ReactNode;accent?:string;back?:boolean;subtitle?:string}){
  return <View style={s.headerWrap}>
    <View pointerEvents="none" style={[s.headerGlow,{backgroundColor:`${accent}16`}]}/>
    <View style={s.header}>
      {back?<Pressable onPress={()=>router.back()} style={({pressed})=>[s.headerButton,pressed&&s.pressed]}><SafeIcon name="arrow-left" size={25} color={palette.text}/></Pressable>:<View style={[s.brand,{borderColor:`${accent}68`,backgroundColor:`${accent}1E`}]}><SafeIcon name="shield-car" size={26} color={accent}/><View style={[s.brandDot,{backgroundColor:accent}]}/></View>}
...
export function AccentCard({icon,title,subtitle,color,onPress,badge}:{icon:string;title:string;subtitle:string;color:string;onPress:()=>void;badge?:string}){
  ...
  return <Animated.View style={{transform:[{scale}]}}><Pressable onPress={onPress} onPressIn={()=>press(.985)} onPressOut={()=>press(1)} style={[s.accentCard,{borderColor:`${color}60`,backgroundColor:`${color}12`}]}><View style={[s.accentGlow,{backgroundColor:`${color}22`}]}/><View style={[s.accentRail,{backgroundColor:color}]}/><IconBubble icon={icon} color={color} size={58}/><View style={{flex:1}}><Text style={s.accentTitle}>{title}</Text><Text style={s.accentSub}>{subtitle}</Text></View>{badge?<View style={[s.badge,{borderColor:`${color}60`,backgroundColor:`${color}20`}]}><Text style={[s.badgeText,{color}]}>{badge}</Text></View>:<View style={[s.next,{borderColor:`${color}58`,backgroundColor:`${color}0D`}]}><SafeIcon name="arrow-right" size={20} color={color}/></View>}</Pressable></Animated.View>;
}
...
export function SectionHeading({title,subtitle,badge,color=palette.cyan}:{title:string;subtitle?:string;badge?:string;color?:string}){
  return <View style={s.sectionHead}><View style={{flex:1}}><View style={s.sectionTitleRow}><View style={[s.sectionDot,{backgroundColor:color}]}/><Text style={s.sectionTitle}>{title}</Text></View>{subtitle?<Text style={s.sectionSub}>{subtitle}</Text>:null}</View>{badge?<View style={[s.sectionBadge,{borderColor:`${color}58`,backgroundColor:`${color}17`}]}><Text style={[s.sectionBadgeText,{color}]}>{badge}</Text></View>:null}</View>;
}

const s=StyleSheet.create({
  base:{...StyleSheet.absoluteFill,backgroundColor:palette.bg},
  orbA:{position:'absolute',width:460,height:460,borderRadius:230,right:-218,top:-165},orbB:{position:'absolute',width:420,height:420,borderRadius:210,left:-250,bottom:8},orbC:{position:'absolute',width:270,height:270,borderRadius:135,right:-125,bottom:180,backgroundColor:`${palette.pink}18`},orbD:{position:'absolute',width:220,height:220,borderRadius:110,left:-120,top:260,backgroundColor:`${palette.green}12`},orbE:{position:'absolute',width:180,height:180,borderRadius:90,right:70,top:420,backgroundColor:`${palette.orange}0E`},
  spectrumBeam:{position:'absolute',top:190,left:-420,width:430,height:16,flexDirection:'row',gap:4,opacity:.18},beamSegment:{flex:1,borderRadius:99},
  spectrumRail:{position:'absolute',top:68,left:-40,flexDirection:'row',gap:25,opacity:.75},railDot:{width:9,height:9,borderRadius:5},
  headerWrap:{borderRadius:28,borderWidth:1,borderColor:palette.lineSoft,backgroundColor:`${palette.glass}`,padding:13,marginBottom:20,overflow:'hidden',...shadows.soft},headerGlow:{position:'absolute',width:180,height:180,borderRadius:90,right:-88,top:-112},header:{flexDirection:'row',alignItems:'center',gap:13},headerButton:{width:48,height:48,borderRadius:17,borderWidth:1,borderColor:palette.line,backgroundColor:palette.panel2,alignItems:'center',justifyContent:'center'},pressed:{opacity:.72,transform:[{scale:.97}]},brand:{width:50,height:50,borderRadius:18,borderWidth:1,alignItems:'center',justifyContent:'center'},brandDot:{position:'absolute',right:6,bottom:6,width:8,height:8,borderRadius:4},eyebrow:{fontSize:type.micro,fontWeight:'900',letterSpacing:1.4},headerTitle:{fontSize:type.title,fontWeight:'900',color:palette.text,letterSpacing:-1,lineHeight:34,marginTop:1},headerSubtitle:{fontSize:type.caption,color:palette.muted,lineHeight:19,marginTop:4},headerSpectrum:{height:4,borderRadius:99,overflow:'hidden',flexDirection:'row',gap:3,marginTop:12},headerColor:{flex:1,borderRadius:99},
  dockWrap:{paddingTop:18,paddingBottom:8},dock:{minHeight:92,borderRadius:31,borderWidth:1,borderColor:palette.line,backgroundColor:palette.glassStrong,paddingHorizontal:7,paddingVertical:9,flexDirection:'row',alignItems:'center',overflow:'hidden',...shadows.floating},dockTopSpectrum:{position:'absolute',left:18,right:18,top:0,height:3,flexDirection:'row',gap:3},dockBar:{flex:1,borderRadius:99},dockItem:{flex:1,minHeight:70,borderRadius:22,borderWidth:1,borderColor:'transparent',alignItems:'center',justifyContent:'center',gap:5},dockIcon:{width:46,height:42,borderRadius:16,borderWidth:1,borderColor:'transparent',alignItems:'center',justifyContent:'center'},activeDot:{position:'absolute',width:5,height:5,borderRadius:3,bottom:4},dockLabel:{fontSize:type.micro,color:palette.muted2,fontWeight:'900'},
  accentCard:{minHeight:110,borderWidth:1,borderRadius:radius.md,padding:16,flexDirection:'row',alignItems:'center',gap:14,overflow:'hidden',...shadows.soft},accentGlow:{position:'absolute',width:190,height:190,borderRadius:95,right:-100,top:-108},accentRail:{position:'absolute',left:0,top:16,bottom:16,width:4,borderRadius:99},accentTitle:{fontSize:type.cardTitle,fontWeight:'900',color:palette.text},accentSub:{fontSize:type.caption,lineHeight:20,color:palette.muted,marginTop:5},badge:{paddingHorizontal:11,paddingVertical:8,borderRadius:999,borderWidth:1},badgeText:{fontSize:type.micro,fontWeight:'900'},next:{width:38,height:38,borderRadius:14,borderWidth:1,alignItems:'center',justifyContent:'center'},
