...
function Loading(){
  ...
  return <SafeAreaView style={s.safe}><AuroraBackground accent={palette.cyan} secondary={palette.purple}/><View style={s.loading}>
    ...
    <Animated.View style={[s.loadingCore,{transform:[{scale:pulse.interpolate({inputRange:[0,1],outputRange:[.95,1.06]})}]}]}><SafeIcon name="car-connected" size={45} color={palette.cyan}/></Animated.View>
...
function Welcome({onDemo}:{onDemo:()=>void}){
  return <SafeAreaView style={s.safe}><AuroraBackground accent={palette.purple} secondary={palette.cyan}/><ScrollView contentContainerStyle={s.welcome} showsVerticalScrollIndicator={false}>
    ...
    <Pressable onPress={()=>router.push('/auth')} style={({pressed})=>[s.primary,pressed&&s.pressed]}><View style={s.primaryIcon}><SafeIcon name="account-arrow-right-outline" size={28} color={palette.ink}/></View><View style={{flex:1}}><Text style={s.primaryTitle}>GİRİŞ YAP / KAYIT OL</Text><Text style={s.primarySub}>Canlı hesabını aç</Text></View><SafeIcon name="arrow-right" size={24} color={palette.ink}/></Pressable>
