...
export default function AccountScreen(){
  ...
  function confirmDelete(){Alert.alert('Hesabını kalıcı olarak sil?','Bu işlem DraBornPark profilini, araçlarını, park kayıtlarını, bildirim geçmişini, Family/Guest verilerini, özel fotoğraflarını ve giriş hesabını kalıcı olarak siler. İşlem geri alınamaz.',[
  ...
  async function performDelete(){setDeleting(true);try{await deleteDraBornParkAccount();Alert.alert('Hesap silindi','DraBornPark hesabın ve hesaba bağlı kullanıcı verilerin kalıcı olarak silindi.',[{text:'Tamam',onPress:()=>router.replace('/')}]);}catch(e:any){Alert.alert('Silme tamamlanamadı',e?.message||'Lütfen tekrar dene. Web üzerinden silme seçeneğini de kullanabilirsin.');}finally{setDeleting(false)}}
  ...
  return <SafeAreaView style={s.safe}><AuroraBackground accent={palette.blue} secondary={palette.purple}/><ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
    ...
    <View style={s.list}>
      <Row icon="shield-lock-outline" color={palette.green} title="Gizlilik & Veri Merkezi" body="Toplanan veriler, kullanım amaçları ve saklama yaklaşımı" onPress={()=>router.push('/legal')}/>
      <Row icon="web" color={palette.cyan} title="Web üzerinden hesap silme" body="Giriş yaparak veya silme talebi oluşturarak kullan" onPress={()=>Linking.openURL(ACCOUNT_DELETION_URL)}/>
      <Row icon="lifebuoy" color={palette.blue} title="DraBornPark Care" body="Destek talebi ve yardım merkezi" onPress={()=>Linking.openURL(SUPPORT_URL)}/>
    ...
    <View style={s.danger}><View style={s.dangerIcon}><SafeIcon name="account-remove-outline" size={30} color={palette.red}/></View><View style={{flex:1}}><Text style={s.dangerTitle}>Hesabımı kalıcı olarak sil</Text><Text style={s.dangerBody}>DraBornPark profilin ve hesaba bağlı kullanıcı verilerin silinir. Etiket fiziksel olarak çalışmaya devam etmez; sahipliği kaldırılır ve yeniden aktivasyon/devir süreci gerekebilir.</Text></View></View>
    <Pressable disabled={deleting} onPress={confirmDelete} style={[s.deleteButton,deleting&&{opacity:.55}]}>{deleting?<ActivityIndicator color={palette.white}/>:<><SafeIcon name="delete-forever-outline" size={24} color={palette.white}/><Text style={s.deleteText}>HESABIMI KALICI OLARAK SİL</Text></>}</Pressable>
    <Text style={s.note}>Silme işlemi, yasal olarak saklanması zorunlu olabilecek kayıtlar dışında DraBornPark kullanıcı verilerini kaldırmak için tasarlanmıştır. Ayrıntılar Gizlilik Politikası'nda açıklanır.</Text>
