...
const dkd_state_info=[
  ['1 • Etiketi üret','Sistem etikete özel kısa kodu ve kalıcı fiziksel NFC/QR URL’sini otomatik oluşturur.'],
  ['2 • QR’ı bas','Ekrandaki QR tam olarak fiziksel NFC/QR bağlantısını içerir. Her fiziksel etiket kendine özel kısa kod taşır.'],
...
export default function FactoryScreen(){
  ...
  async function dkd_save_edit(dkd_tag:any){
    ...
    if(!/^\d{4}-\d{4}$/.test(dkd_short))return Alert.alert('Kısa kod geçersiz','Kısa fiziksel etiket kodu 1234-4321 biçiminde olmalıdır.');
    ...
    try{
      ...
      Alert.alert('Etiket güncellendi','Kısa fiziksel NFC/QR bağlantısı kaydedildi. Kullanıcı adı varsa ayrıca kişisel paylaşım bağlantısı da aynı aktif etiketi çözer.');
    }catch(dkd_problem:any){const dkd_message=String(dkd_problem?.message||'');Alert.alert('Etiket düzenlenemedi',dkd_message.includes('username_taken')?'Bu kullanıcı adı kullanımda.':dkd_message.includes('unique')||dkd_message.includes('reserved')?'Etiket ID, seri, kısa kod veya kullanıcı adı başka bir kayıtta kullanılıyor.':dkd_message||'Lütfen tekrar dene.')}finally{setDkdBusy(false)}
  }
  ...
  return <SafeAreaView style={s.safe}><AuroraBackground accent={palette.cyan} secondary={palette.orange}/><ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <ScreenHeader title="Üretim Paneli" eyebrow="NFC + QR ETİKET MERKEZİ • v1.0.4" accent={palette.cyan} subtitle="Fiziksel etiket kısa kodu kalıcıdır; sahip ve araç eşleşmesi Supabase üzerinden dinamik değişir."/>
    <View style={s.stats}><DkdStat icon="nfc" value={dkd_tags.length} label="Toplam" color={palette.cyan}/><DkdStat icon="qrcode" value={dkd_tags.filter(dkd_tag=>dkd_tag.public_alias).length} label="Kısa URL" color={palette.purple}/><DkdStat icon="shield-check" value={dkd_active_count} label="Aktif" color={palette.green}/></View>
    ...
    <SectionHeading title="Yeni etiket üret" subtitle="Sistem otomatik benzersiz 1234-4321 kısa kod üretir"/>
    ...
    <SectionHeading title="Üretim kuyruğu" subtitle="Kısa kod, kullanıcı adı ve fiziksel NFC/QR eşleşmesini yönet" badge={`${dkd_tags.length} ETİKET`} color={palette.orange}/>
