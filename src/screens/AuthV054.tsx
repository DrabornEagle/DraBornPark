...
const PENDING_AVATAR_KEY='drabornpark_pending_avatar_v054';
...
export default function AuthV054(){
  ...
  async function pickAvatar(){
    try{
      ...
      Alert.alert('Profil resmi açılamadı',raw.includes('ActivityResultLauncher')?'Bu cihazdaki eski Developer APK fotoğraf seçici modülünü yüklememiş. DraBornPark v0.5.4 Developer APK kurulduğunda yeniden dene.':'Fotoğraf arşivi açılamadı. İstersen profil resmini daha sonra Hesabım üzerinden ekleyebilirsin.');
    }
  }

  async function uploadPendingAvatar(asset:any){
    if(!asset)return;
    try{await uploadProfileAvatar({uri:asset.uri,mimeType:asset.mimeType,fileName:asset.fileName});await AsyncStorage.removeItem(PENDING_AVATAR_KEY);}
    catch{await AsyncStorage.setItem(PENDING_AVATAR_KEY,JSON.stringify({uri:asset.uri,mimeType:asset.mimeType,fileName:asset.fileName}));}
  }
  ...
  async function submit(){
    ...
    try{
      if(mode==='login'){
        ...
        if(!/^[a-z0-9._-]{3,24}$/.test(normalized)){Alert.alert('Kullanıcı adını kontrol et','Kullanıcı adı 3–24 karakter olmalı; yalnızca küçük harf, rakam, nokta, alt çizgi ve tire kullanabilirsin.');return;}
