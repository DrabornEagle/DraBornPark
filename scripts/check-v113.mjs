...
if(!Object.values(dkd_pkg.scripts??{}).every(dkd_script=>String(dkd_script).includes('apply-v113-safe.mjs')))dkd_fail.push('All project scripts must apply repeat-safe v1.0.13 source transform');
for(const dkd_file of ['app/admin.tsx','src/components/DraBornParkPlusPanel.tsx','src/components/MandatoryUpdateGate.tsx','src/lib/drabornpark.ts','scripts/apply-v113-safe.mjs','scripts/apply-v113-source-transform.mjs','supabase/functions/dkd-drabornpark-app-version/index.ts'])if(!dkd_exists(dkd_file))dkd_fail.push('Required file missing: '+dkd_file);

const dkd_safe=dkd_read('scripts/apply-v113-safe.mjs');
for(const dkd_marker of ['dkd_v113_applied','apply-v113-source-transform.mjs','repeat-safe skip'])if(!dkd_safe.includes(dkd_marker))dkd_fail.push('Repeat-safe transform marker missing: '+dkd_marker);
...
console.log('DraBornPark integrity OK • v1.0.13 • Android vc13 • repeat-safe transforms • entitlement flicker blocked • refunded Play ownership removed • admin force-update control ready.');
