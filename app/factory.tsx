...
const dkd_state_info=[
  ['1 • Etiketi üret','Sistem etikete özel NFC bağlantı kodunu ve kalıcı NFC/QR URL’sini otomatik oluşturur.'],
  ['2 • QR’ı bas','QR yalnızca https://www.draborneagle.com/DraBornPark/tag/1234-5678 biçimindeki kısa bağlantıyı içerir.'],
  ['3 • NTAG213’e yaz','Aynı kısa URL’yi NDEF URL/URI kaydı olarak NTAG213’e yaz. UUID, kişi, telefon veya plaka çipe yazılmaz.'],
  ['4 • Test et','Telefonla NFC’yi, kamerayla QR’ı okut. İkisi de aynı kısa etiket sayfasına gitmelidir.'],
  ['5 • Aktivasyon','Müşteri PIN ile etiketi hesabına ve aracına bağlar. Fiziksel kısa URL değişmeden backend eşleşmesi güncellenir.'],
];

type DkdEdit={tagCode:string;serial:string;nfcCode:string;pin:string;notes:string};
const dkd_empty_edit:DkdEdit={tagCode:'',serial:'',nfcCode:'',pin:'',notes:''};
...
export default function FactoryScreen(){
  ...
  async function dkd_create(){
    ...
    try{
      ...
      await dkd_refresh();
      Alert.alert('Yeni etiket üretildi','Etikete özel kısa NFC bağlantısı oluşturuldu. QR aynı adresi içerir; NTAG213’e de yalnızca bu kısa URL yazılacaktır.');
  ...
  function dkd_begin_edit(dkd_tag:any){
    ...
    setDkdEdit({tagCode:dkd_tag.tag_code||'',serial:dkd_tag.serial_number||'',nfcCode:dkd_tag.public_alias||dkd_short_from_url(dkd_tag.nfc_url),pin:'',notes:dkd_tag.factory_notes||''});
  }

  async function dkd_save_edit(dkd_tag:any){
    const dkd_tag_code=dkd_edit.tagCode.trim().toUpperCase();
    const dkd_nfc_code=dkd_edit.nfcCode.trim().toLowerCase();
    ...
    if(!/^\d{4}-\d{4}$/.test(dkd_nfc_code))return Alert.alert('NFC bağlantı kodu geçersiz','NFC bağlantı kodu 1234-4321 biçiminde olmalıdır.');
    ...
    setDkdBusy(true);
    try{
      const {error}=await supabase.rpc('dkd_drabornpark_factory_update_tag_v105',{dkd_tag_id:dkd_tag.id,dkd_tag_code,dkd_serial_number:dkd_edit.serial.trim()||null,dkd_nfc_connection_code:dkd_nfc_code,dkd_activation_pin:dkd_pin||null,dkd_factory_notes:dkd_edit.notes.trim()||null});
      if(error)throw error;
      ...
      Alert.alert('Etiket güncellendi','NFC ve QR için tek kısa bağlantı kaydedildi. Uzun UUID bağlantısı üretilmez.');
    }catch(dkd_problem:any){const dkd_message=String(dkd_problem?.message||'');Alert.alert('Etiket düzenlenemedi',dkd_message.includes('unique')||dkd_message.includes('reserved')?'Etiket ID, seri veya NFC bağlantı kodu başka bir kayıtta kullanılıyor.':dkd_message||'Lütfen tekrar dene.')}finally{setDkdBusy(false)}
  }
  ...
  return <SafeAreaView style={s.safe}><AuroraBackground accent={palette.cyan} secondary={palette.orange}/><ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <ScreenHeader title="Üretim Paneli" eyebrow="NFC + QR ETİKET MERKEZİ • v1.0.5" accent={palette.cyan} subtitle="NFC bağlantı kodu kalıcıdır; sahip ve araç eşleşmesi Supabase üzerinden dinamik değişir."/>
    ...
    <SectionHeading title="Doğru fiziksel akış" subtitle="Her etiket tek kısa NFC bağlantısına sahiptir" color={palette.purple}/>
    ...
    {dkd_last_secret?<View style={s.secret}><View style={s.secretHead}><MaterialCommunityIcons name="key-variant" size={30} color={palette.yellow}/><View style={{flex:1}}><Text style={s.secretTitle}>Tek seferlik üretim paketi</Text><Text style={s.muted}>QR ve NTAG213’e yazacağın kısa URL birebir aynıdır.</Text></View></View>
      ...
      <Text style={s.warning}>PIN yalnızca üretim anında düz metin görünür. Fiziksel QR/NFC içine PIN, UUID, telefon, plaka veya kullanıcı kimliği yazılmaz.</Text>
    </View>:null}

    <SectionHeading title="Üretim kuyruğu" subtitle="NFC bağlantı kodunu ve fiziksel NFC/QR eşleşmesini yönet" badge={`${dkd_tags.length} ETİKET`} color={palette.orange}/>
    <View style={s.list}>{dkd_tags.map(dkd_tag=>{const dkd_editing=dkd_editing_id===dkd_tag.id;const dkd_url=dkd_physical_url(dkd_tag.public_alias||dkd_short_from_url(dkd_tag.nfc_url));return <View key={dkd_tag.id} style={s.tag}>
      <View style={s.tagHead}><View style={s.nfc}><MaterialCommunityIcons name="nfc" size={29} color={palette.cyan}/></View><View style={{flex:1,minWidth:0}}><Text style={s.tagCode}>{dkd_tag.tag_code}</Text><Text style={s.tagMeta}>{dkd_tag.serial_number} • NFC {dkd_tag.public_alias||'Kod yok'}</Text><Text numberOfLines={2} selectable style={s.tagUrl}>{dkd_url}</Text></View><View style={s.badge}><Text style={s.badgeText}>{dkd_state_labels[dkd_tag.status]||dkd_tag.status}</Text></View></View>
      <View style={s.quickActions}><Pressable onPress={()=>void dkd_copy(dkd_url,'NFC/QR bağlantısı')} style={s.secondary}><MaterialCommunityIcons name="content-copy" size={18} color={palette.cyan}/><Text style={s.secondaryText}>NFC/QR KOPYALA</Text></Pressable><Pressable onPress={()=>dkd_editing?setDkdEditingId(null):dkd_begin_edit(dkd_tag)} style={s.secondary}><MaterialCommunityIcons name={dkd_editing?'close':'pencil-outline'} size={18} color={palette.purple}/><Text style={s.secondaryText}>{dkd_editing?'KAPAT':'DÜZENLE'}</Text></Pressable><Pressable disabled={dkd_busy} onPress={()=>dkd_remove(dkd_tag)} style={[s.secondary,{borderColor:`${palette.red}66`}]}><MaterialCommunityIcons name="trash-can-outline" size={18} color={palette.red}/></Pressable></View>
      {dkd_editing?<View style={s.editor}><Text style={s.editorTitle}>Etiket üretim bilgilerini düzenle</Text>
        ...
        <Text style={s.label}>NFC BAĞLANTI KODU • FİZİKSEL NFC/QR</Text><TextInput style={s.input} keyboardType="number-pad" maxLength={9} value={dkd_edit.nfcCode} onChangeText={dkd_value=>{const dkd_digits=dkd_value.replace(/\D/g,'').slice(0,8);setDkdEdit(dkd_current=>({...dkd_current,nfcCode:dkd_digits.length>4?`${dkd_digits.slice(0,4)}-${dkd_digits.slice(4)}`:dkd_digits}))}} placeholder="1234-4321" placeholderTextColor={palette.muted2}/>
        <Text style={s.derived}>{dkd_physical_url(dkd_edit.nfcCode)}</Text><DkdQr url={dkd_physical_url(dkd_edit.nfcCode)} onCopy={()=>void dkd_copy(dkd_physical_url(dkd_edit.nfcCode),'NFC/QR bağlantısı')}/>
